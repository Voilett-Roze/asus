...
* _… you must save files with a UNIX encoding. Note that Windows's Notepad cannot save with a UNIX encoding - get Notepad++ instead. You can also directly edit them on the router through SSH, by using vi or nano, both included in the firmware. These two will create files already encoded in the proper format._

So for Windows users, one quick option is to run nano via telnet or ssh, then just copy/paste the contents of the above script into the nano window, and finally save as /jffs/scripts/pre-mount.
* Make the script executable (_chmod a+rx /jffs/scripts/pre-mount_)  [I can't tell you how many times I've forgotten that step and am left scratching my head wondering why it doesn’t work!]
...
Anyway, as with any religious conflict, feel free to go with whichever method feels right to you.  Just offering this alternative.
...
Install as '/jffs/scripts/unmount' (caveats for installing scripts mentioned above apply):
    ...
    if [ "$1" == "$OPT" ] ; then
            # services-stop already does exactly what we want, just reuse it here
