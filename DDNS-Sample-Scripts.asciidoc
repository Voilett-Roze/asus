...
This updates the `@` and `www` subdomains of a type `A` record while leaving any others intact. Change the SUBDOMAIN variable to update a specific A Record.
...
curl --location --fail --silent --output /dev/null \
     --request PUT 'https://api.gandi.net/v5/livedns/domains/${DOMAIN}/records/${SUBDOMAIN}/A' \
