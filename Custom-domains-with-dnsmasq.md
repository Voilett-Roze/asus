...
4. Go to `LAN -> DNS Filter` (`Parental Controls -> DNSFilter` on older models) (`LAN -> DNS Director` on firmware version >= 388.1)
...
4. Go to `WAN -> Internet Connection`. Turn off `Forward local domain queries to upstream DNS` to prevent your private DNS resolution requests from being passed to the Internet.
